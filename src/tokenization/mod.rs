...
pub fn tokenize<'src>(src: &'src str, errs: &mut Vec<Error>) -> TokenStream<'src> {
...
fn read_group<'a>(open: RawToken, tokens: &mut RawTokenIter, errs: &mut Vec<Error>) -> ReadGroupOutput {
    let open = GroupEdge {
